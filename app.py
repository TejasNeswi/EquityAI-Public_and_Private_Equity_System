...
entry_val = st.number_input("Entry Valuation (Million USD)", value=120.0)
ownership = st.number_input("Ownership Percent (%)", value=12.0)
growth = st.number_input("Revenue Growth (%)", value=40.0)
burn = st.number_input("Burn Rate (Million USD/year)", value=6.0)
...
if st.button("🚀 Simulate Returns"):
    ...
    if private_equity_model:
        ...
        st.write(f"Expected Final Value: {pe_results.mean():.2f} Million USD")
        st.write(f"Median: {np.median(pe_results):.2f} Million USD")
        st.write(f"5th percentile (pessimistic): {np.percentile(pe_results, 5):.2f} Million USD")
        st.write(f"95th percentile (optimistic): {np.percentile(pe_results, 95):.2f} Million USD")
    ...
    if public_equity_model:
        ...
        st.write(f"Expected Final Value: {pub_results.mean():.2f} Million USD")
        st.write(f"Median: {np.median(pub_results):.2f} Million USD")
        st.write(f"5th percentile (pessimistic): {np.percentile(pub_results, 5):.2f} Million USD")
        st.write(f"95th percentile (optimistic): {np.percentile(pub_results, 95):.2f} Million USD")
